...
export async function POST(req) {
  try {
    ...
    if (!nome?.trim() || !email || !senha || !perfil) {
    ...
    // Uma única query: perfil + empresa_id
    ...
    if (empresa_id) registro.empresa_id = empresa_id

    const { error: dbErr } = await supaAdmin.from('usuarios').upsert(registro, { onConflict: 'id' })
